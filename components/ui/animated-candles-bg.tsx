...
export function AnimatedCandlesBg() {
  ...
  useEffect(() => {
    ...
    const drawCandle = (candle: Candle) => {
      ...
      // Colors
      const bodyColor = isGreen ? `rgba(34, 197, 94, ${opacity})` : `rgba(239, 68, 68, ${opacity})`
      const wickColor = isGreen ? `rgba(34, 197, 94, ${opacity * 0.7})` : `rgba(239, 68, 68, ${opacity * 0.7})`
      ...
      // Draw border
      ctx.strokeStyle = isGreen ? `rgba(74, 222, 128, ${opacity * 1.5})` : `rgba(248, 113, 113, ${opacity * 1.5})`
      ctx.lineWidth = 1
