  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-[#121826] border border-[#1f2933] hover:border-[#22c55e]/50 transition-all"
      >
        ...
        <div className="flex flex-col items-start">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[#22c55e] font-mono text-xs font-medium">{formatPrice(currentPrice)}</span>
            <span className={`text-xs flex items-center gap-0.5 ${isUp ? "text-[#4ade80]" : "text-[#EF4444]"}`}>
            ...
            <div className="p-3 border-b border-[#1f2933]">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-[#0B0F14] border border-[#1f2933] rounded-lg text-white text-sm placeholder:text-[#6B7280] focus:outline-none focus:border-[#22c55e]"
                />
              </div>
            </div>
            ...
            <div className="flex gap-1 p-2 border-b border-[#1f2933] overflow-x-auto">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap ${
                  !activeCategory ? "bg-[#22c55e] text-white" : "bg-[#1f2933] text-[#9CA3AF] hover:text-white"
                }`}
              >
                Todos
              </button>
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap ${
                    activeCategory === cat ? "bg-[#22c55e] text-white" : "bg-[#1f2933] text-[#9CA3AF] hover:text-white"
                  }`}
                >
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-[#1f2933] transition-colors ${
                        isSelected ? "bg-[#22c55e]/10 border-l-2 border-[#22c55e]" : "border-l-2 border-transparent"
                      }`}
                    >
                      ...
                      <div className="text-right">
                        <div className={`font-medium text-sm ${isClosed ? "text-white/30" : "text-[#22c55e]"}`}>
                          {asset.payout || 96}%
                        </div>
